...
<div class="mb-6">
    ...
    <p class="text-slate-400">Database management and system tools</p>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="bg-slate-800 rounded-xl shadow-lg border border-slate-700 p-6">
        <div class="flex items-center gap-4 mb-4">
            ...
            <div>
                <h2 class="text-lg font-bold text-white">Backup Database</h2>
                <p class="text-sm text-slate-400">Download a full SQL dump of the database.</p>
            </div>
        </div>
        <p class="text-slate-500 text-sm mb-6">
            This will generate a downloadable SQL file containing all your current data (Books, Users, Loans, Settings).
        </p>
    ...
    <!-- Restore Database -->
    <div class="bg-slate-800 rounded-xl shadow-lg border border-blue-500/20 p-6">
        <div class="flex items-center gap-4 mb-4">
            <div class="w-12 h-12 rounded-lg bg-blue-500/20 flex items-center justify-center text-blue-400">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
            </div>
            <div>
                <h2 class="text-lg font-bold text-white">Restore Database</h2>
                <p class="text-sm text-slate-400">Restore system from a SQL backup file.</p>
            </div>
        </div>
        <p class="text-slate-500 text-sm mb-6">
            Upload a <strong>.sql</strong> file to restore the database. <br>
            <span class="text-yellow-500 font-bold">Warning:</span> Existing data will be updated/replaced by the backup file.
        </p>
        <form action="{{ route('system.restore') }}" method="POST" enctype="multipart/form-data" class="space-y-4" onsubmit="return confirm('Are you sure you want to restore this database backup? Current data might be overwritten.');">
            @csrf
            <div>
    ...
    <!-- Reset Database -->
    <div class="bg-slate-800 rounded-xl shadow-lg border border-red-500/20 p-6 relative overflow-hidden">
        ...
        <div class="flex items-center gap-4 mb-4">
            ...
            <div>
                <h2 class="text-lg font-bold text-white">Reset Database</h2>
                <p class="text-sm text-red-400">Warning: Destructive Action!</p>
            </div>
        </div>
        <p class="text-slate-400 text-sm mb-6">
            This will <strong class="text-red-400">WIPE ALL DATA</strong> and re-seed the database with default initial data. This action cannot be undone unless you have a backup.
        </p>
        <form action="{{ route('system.reset') }}" method="POST" onsubmit="return confirm('CRITICAL WARNING: Are you absolutely sure you want to delete ALL DATA and reset the system?');">
            @csrf
