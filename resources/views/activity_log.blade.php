    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-white">
                {{ $isSuperAdmin ? 'Log Aktivitas Sistem' : 'Aktivitas Saya' }}
            </h1>
            <p class="text-slate-400 mt-1">
                {{ $isSuperAdmin ? 'Pantau semua aktivitas pengguna, login, logout, dan perubahan data.' : 'Riwayat login dan aktivitas akun Anda.' }}
            </p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('activity.log') }}" class="p-2 bg-slate-800 text-slate-300 rounded-lg hover:bg-slate-700 hover:text-white transition-colors" title="Refresh">
            ...
            <div class="flex-1">
                <div class="relative">
                    ...
                    <input type="text" name="q" value="{{ request('q') }}" 
                        class="w-full bg-slate-900 border-slate-700 rounded-lg pl-10 pr-4 py-2.5 text-sm text-white placeholder-slate-500 focus:ring-2 focus:ring-indigo-500 focus:border-transparent" 
                        placeholder="{{ $isSuperAdmin ? 'Cari nama, IP, aktivitas...' : 'Cari aktivitas Anda...' }}">
                </div>
            </div>
            ...
            <table class="w-full text-left text-sm text-slate-400">
                <thead class="bg-slate-900/50 text-xs uppercase font-semibold text-slate-300">
                    <tr>
                        ...
                        <th class="px-6 py-4">Info Device</th>
                ...
                <tbody class="divide-y divide-slate-700">
                    ...
                    <tr class="hover:bg-slate-700/30 transition-colors">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-white font-medium">{{ \Carbon\Carbon::parse($log->created_at)->format('d M Y') }}</div>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="text-white font-medium">{{ $log->user_name ?? 'Unknown User' }}</div>
                        ...
                        <td class="px-6 py-4">
                            ...
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $badgeColor }}">
                                {{ strtoupper($log->action_type ?? 'General') }}
                            </span>
                            <div class="text-xs text-slate-500 mt-1">Table: {{ $log->table_name }} #{{ $log->row_id }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="text-xs">
                                ...
                                <div title="{{ $log->user_agent }}"><span class="text-slate-500">Agent:</span> {{ Str::limit($log->user_agent, 20) ?? '-' }}</div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            ...
                            @elseif($log->action_type == 'location_ping')
                                <span class="text-yellow-500 text-xs">Waiting GPS...</span>
                            @else
