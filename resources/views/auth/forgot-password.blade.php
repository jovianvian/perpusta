...
<div class="text-center mb-8">
    ...
    <p class="text-slate-400 mt-2">Pilih metode OTP untuk reset password akun</p>
</div>
...
<form method="POST" action="{{ route('password.email') }}" class="space-y-6">
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <label class="cursor-pointer border border-slate-600 rounded-lg p-3 flex items-center justify-center gap-2 hover:bg-slate-700 transition-colors has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-500/10 has-[:checked]:text-indigo-400">
                    <input type="radio" name="method" value="email" class="hidden" {{ (old('method', request('method', 'email')) === 'email') ? 'checked' : '' }} onchange="toggleMethod('email')">
                ...
                <label class="cursor-pointer border border-slate-600 rounded-lg p-3 flex items-center justify-center gap-2 hover:bg-slate-700 transition-colors has-[:checked]:border-emerald-500 has-[:checked]:bg-emerald-500/10 has-[:checked]:text-emerald-400">
                    <input type="radio" name="method" value="whatsapp" class="hidden" {{ (old('method', request('method', 'email')) === 'whatsapp') ? 'checked' : '' }} onchange="toggleMethod('whatsapp')">
    ...
    <button id="submitOtpButton" type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-lg transition-colors shadow-lg shadow-indigo-600/30">
        Kirim OTP
    </button>
...
<script>
    function toggleMethod(method) {
        ...
        if (method === 'email') {
            ...
            submitButton.textContent = 'Kirim OTP ke Email';
        } else {
            ...
            submitButton.textContent = 'Kirim OTP ke WhatsApp';
        }
    }

    document.addEventListener('DOMContentLoaded', function () {
        toggleMethod(@json(old('method', request('method', 'email'))));
    });
</script>
@endsection
