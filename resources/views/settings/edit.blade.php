...
<div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">{{ __('Edit Permissions') }}</h1>
        <p class="text-slate-600 dark:text-slate-400">
            {{ __('Configure access rights for role:') }}
            <span class="text-indigo-400 font-bold">{{ $level->nama_level }}</span>
        </p>
    </div>
    <a href="{{ route('settings.index') }}" class="bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-900 dark:text-white font-medium py-2 px-4 rounded-lg transition-colors flex items-center gap-2">
    ...
    <form action="{{ route('settings.update', $level->id) }}" method="POST">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
                <!-- Module Header -->
                <div class="px-4 py-3 bg-slate-100 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
                    <h3 class="font-bold text-slate-900 dark:text-white uppercase tracking-wider text-sm">
                    ...
                    <label class="flex items-center gap-2 cursor-pointer group">
                        ...
                        <span class="text-xs text-indigo-500 dark:text-indigo-400 font-medium group-hover:text-indigo-400 dark:group-hover:text-indigo-300 transition-colors">{{ __('Select All') }}</span>
                    </label>
                </div>
                ...
                <div class="p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div class="flex items-start gap-3 p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-colors">
                        ...
                        <label for="perm_{{ $perm->id }}" class="text-sm text-slate-700 dark:text-slate-300 cursor-pointer select-none leading-5">
                            {{ $perm->description }}
                        </label>
                    </div>
                    @endforeach
